...
class Deck {
    ...
    public Deck(int n)
    {
    	originalDeck = new ArrayList<Card>(52*n);
		playingDeck = new ArrayList<Card>(52*n);
		numberDecks = n;
		doDeck(numberDecks);
    }
    ...
    private void doDeck(int numberDecks){
    	try{
    		for(int i= 0; i< numberDecks; i++){
    			for(int k = 1; k < 5; k++){
    				for(int j = 1; j < 14; j++){
    					originalDeck.add(new Card(j,k));
    				}
    			}
    		} playingDeck = originalDeck;
    	}
    	catch(PlayingCardException e){
    		System.out.println("PlayingCardException: " + e.getMessage());
    	}
    ...
    public List<Card> deal(int numberCards) throws PlayingCardException
    {
        ...
        for (int i = 0; i < numberCards; i++) {
            dealtCards.add(playingDeck.remove(0));
        }
        return dealtCards;
    }
